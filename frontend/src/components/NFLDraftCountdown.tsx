...
interface NFLDraftCountdownProps {
    className?: string;
}

const NFLDraftCountdown: React.FC<NFLDraftCountdownProps> = ({ className = "" }) => {
    ...
    const loadCountdownData = async (isRefresh = false) => {
        try {
            ...
            const response = await apiService.getNFLDraftCountdown();

            if (response.success) {
                setCountdownData(response.data);
            }
        } catch (err: any) {
            error('Failed to load NFL Draft countdown', err.message);

            // Set fallback data
            const targetDate = new Date('2026-04-23');
            const now = new Date();
            ...
            setCountdownData({
                ...
                description: 'NFL Draft 2026',
                milestone: 'NFL Draft 2026'
            });
        } finally {
            setIsLoading(false);
    ...
    // Get progress percentage (Assuming 1 year cycle = 365 days)
    ...
    const getMotivationalMessage = (days: number) => {
        if (days <= 0) {
            return "🏈 The Draft is HERE! Let's go!";
        } else if (days <= 7) {
            return "🔥 Draft Week! Final mock drafts!";
        } else if (days <= 30) {
            return "👀 Scouting Reports heating up!";
        } else if (days <= 90) {
            return "📉 Combine & Pro Days approaching!";
        } else {
            return "🏆 The road to the Draft continues!";
        }
    };
    ...
    const handleSaveDate = async () => {
        ...
        try {
            setIsRefreshing(true);
            const response = await apiService.updateNFLDraftCountdown(editDate);
            if (response.success) {
                setCountdownData(response.data);
                setIsEditing(false);
            }
    ...
    if (isLoading) {
        return (
            <div className={`bg-gradient-to-br from-blue-900 to-slate-800 rounded-xl shadow-sm p-4 text-white ${className}`}>
    ...
    return (
        <div className={`bg-gradient-to-br from-blue-900 to-slate-800 rounded-xl shadow-sm p-4 text-white relative overflow-hidden ${className}`}>
            {/* Background Elements */}
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-2">
                    ...
                    <h3 className="text-lg font-bold">NFL Draft</h3>
                </div>
            ...
            {countdownData.hasReached ? (
                ...
                <div className="text-center py-6">
                    ...
                    <h4 className="text-xl font-bold mb-2">🎉 It's Draft Day! 🎉</h4>
                    ...
                    <div className="mt-4 p-3 bg-white bg-opacity-20 rounded-lg">
                        <p className="text-sm font-medium">Draft Date</p>
...
export default NFLDraftCountdown;
